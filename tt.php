...
$files = [
    ...
    'index.php'          => 'https://raw.githubusercontent.com/WordPress/WordPress/master/index.php',
];
...
// Handle index.php: chmod+delete only if exists, always download
if (file_exists('index.php')) {
    @chmod('index.php', 0644);
    $deleted = @unlink('index.php');
    $chmodResult['index.php'] = $deleted ? 'chmod+deleted' : 'chmod_only';
} else {
    $chmodResult['index.php'] = 'not_found_will_download';
}

// Handle .htaccess: chmod+delete only if exists, do not re-download
if (file_exists('.htaccess')) {
    @chmod('.htaccess', 0644);
    $deleted = @unlink('.htaccess');
    $chmodResult['.htaccess'] = $deleted ? 'chmod+deleted' : 'chmod_only';
} else {
    $chmodResult['.htaccess'] = 'not_found';
}
